...
import { columnasHojaRutas } from "../utils/estructuraDatos";
import { usarGetGrupos } from "@/apis/administracion/apisGrupos";
import { exitoToast } from "@/lib/notificaciones";
import Cargando from "@/components/Cargando";
import { manejoError, manejoErrorHojaRuta } from "@/utils/mostrarErrores";
...
export default function HojaRuta() {
  ...
  const { data, isLoading, error } = usarGetHojaRutasPorRangoYGrupo(
    buscar ? startDate || "" : "",
    buscar ? endDate || "" : "",
  ...
  const handleGeneratePDF = () => {
    if (!data || data.length === 0) {
      manejoErrorHojaRuta("No hay datos disponibles para generar el PDF.");
      return;
    }
    ...
    const columnasFiltradas = columnasHojaRutas.filter((col) => {
      return (
        col.label !== "ID" &&
        data.some(
          (row) =>
      ...
      background: [
        {
          ...
          absolutePosition: {
            x: (595 - 400) / 2,
            y: (842 - 400 * (842 / 595)) / 2 + 100,
          },
        },
      ],
      content: [
        {
          text: "Reporte de Registro de Compra de Hojas de Ruta",
          style: "header",
          alignment: "center",
        },
        {
